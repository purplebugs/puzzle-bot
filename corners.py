...
#return the four corners
    ...
    for i in range(len(lines)):
        for j in range(i + 1, len(lines)):
            ...
            min_delta_angle = min(abs(a['angle'] - b['angle']), abs((a['angle'] - 180) - b['angle']), abs(a['angle'] - b['angle'] - 180))
            if min_delta_angle < 5: # and minimum_length > 0.5 * min(component['width'], component['height']):
    ...
    for candidate in candidates:
        print(f"Candidate: {candidate['l1']['p1']} -> {candidate['l1']['p2']} | {candidate['l2']['p1']} -> {candidate['l2']['p2']} | Delta Angle: {candidate['delta_angle']} | Length: {candidate['length']}")
    ...
    for i in range(len(candidates)):
        ...
        for j in range(i + 1, len(candidates)):
            ...
            if set(points_A) == set(points_B):
                ...
                angle_diff = abs(angle_A1 - angle_B1)
    ...
    for candidate_pair in candidate_pairs:
        print(f"Candidate pair: {candidate_pair['c1']['l1']['p1']} -> {candidate_pair['c1']['l1']['p2']} | {candidate_pair['c2']['l1']['p1']} -> {candidate_pair['c2']['l1']['p2']}")
